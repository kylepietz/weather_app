...
app.controller('weather_ctrl', function($scope, $http) {
    ...
    $scope.locations = [];
    $scope.displayedLoc = {city: null, state: null};
    ...
    var retrieveWeatherInfo = function(loc, fromDB) {
        var c = loc.city;
        var s = loc.state;
        return $http.get("http://api.wunderground.com/api/"+myKey+"/conditions/q/"+s+"/"+c+"/.json")
    ...
    $scope.addLocation = function(displayedLoc) {
        if ($scope.displayedLoc.city === null || $scope.displayedLoc.state === null) {
            ...
            var addedLoc = {city: $scope.displayedLoc.city, state: $scope.displayedLoc.state};
